...
from products.dtos.product_dto import ProductDto, ProductOptionGroupDto
...
async def create_product_option_group(
        option_group_dto: ProductOptionGroupDto,
        current_user: MerchantUser = Depends(get_current_user),
        db: Session = Depends(get_db)):
    ...
    option_group = ProductOptionGroup(
        **option_group_dto.model_dump(exclude=["options"]),
        merchant_id=current_user.merchant_id
    )
    db.add(option_group)
    db.commit()

    for option_dto in option_group_dto.options:
        option = ProductOption(
            **option_dto.model_dump(exclude=['image_path']),
            product_option_group_id=option_group.id
        )
    ...
    return ProductDto.model_validate(option_group)
